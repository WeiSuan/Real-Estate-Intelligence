...
# 讀取 table1_all_approval_cases.xlsx
all_approval = pd.read_excel('./Result/截至20260430/table1_all_approval_cases.xlsx', sheet_name = "RawData")
...
all_approval['approved_amount'] = pd.to_numeric(all_approval['approved_amount'], errors='coerce')
all_approval['approved_amount'] = all_approval['approved_amount'].where(~all_approval['approved_amount'].isnull(), 'NA')


# 讀取 table2_primary_key_mapping.xlsx 的 sheetName = "代碼對應表"
key_mapping = pd.read_excel('./Result/截至20260430/table2_primary_key_mapping.xlsx', sheet_name='代碼對應表')
for col in ['pre_examine_no', 'pre_examine_no_return']:
    key_mapping[col] = pd.to_numeric(key_mapping[col], errors='coerce').astype('Int64').astype(str)
key_mapping['total_approved_amount'] = pd.to_numeric(key_mapping['total_approved_amount'], errors='coerce')
key_mapping['total_approved_amount'] = key_mapping['total_approved_amount'].where(~key_mapping['total_approved_amount'].isnull(), 'NA')
...
loan_remain = pd.read_excel('./Result/截至20260430/table3_loan_remain_cases.xlsx', sheet_name='RawData')
...
# (初審)擔保率
pre_worth_rate = pd.read_excel('./Result/截至20260430/table2_primary_key_mapping.xlsx', sheet_name='初審單案擔保率')
pre_worth_rate["pre_examine_no"] = pd.to_numeric(pre_worth_rate["pre_examine_no"], errors='coerce').astype('Int64').astype(str)
pre_worth_rate = pre_worth_rate[["pre_examine_no", "pre_immovable_worth", "pre_immovable_worth_rate"]].drop_duplicates()

# (合約)擔保率
loan_worth_rate = pd.read_excel('./Result/截至20260430/table2_primary_key_mapping.xlsx', sheet_name='合約單案擔保率(已撥)')
loan_worth_rate["loan_no"] = pd.to_numeric(loan_worth_rate["loan_no"], errors='coerce').astype('Int64').astype(str)
loan_remain_merged = pd.merge(loan_remain_merged, loan_worth_rate.drop(columns = "loan_capital"), on = 'loan_no', how = 'left')

# 當 loan_remain_merge 的 immovable_worth 或 immovable_worth_rate 為空時，補 pre_worth_rate 對應 pre_examine_no 的 pre_immovable_worth/pre_immovable_worth_rate
# 1. 先將 pre_examine_no = "<NA>" 的資料切出
loan_remain_null = loan_remain_merged[loan_remain_merged['pre_examine_no'].isin(["<NA>"]) & \
                                      ~loan_remain_merged['loan_code_type'].isin(["(2)建融"])].copy()

# 2. 將 pre_examine_no 不為 '<NA>' 且 loan_code_type == "(2)建融" 的資料切出
loan_remain_notnull_construction = loan_remain_merged[loan_remain_merged["loan_code_type"].isin(["(2)建融"])].copy()
loan_remain_notnull_construction['immovable_worth'] = 0.
loan_remain_notnull_construction['immovable_worth_rate'] = 0.


# 3. 將 pre_examine_no 不為 '<NA>' 且 loan_code_type <> "(2)建融" 的資料切出
loan_remain_notnull_notconstruction = loan_remain_merged[~(loan_remain_merged['pre_examine_no'].isin(["<NA>"]) | \
                                                           loan_remain_merged["loan_code_type"].isin(["(2)建融"]))].copy()

loan_remain_notnull_notconstruction = pd.merge(
...
loan_immovable_gage = pd.read_excel('./Result/截至20260430/table4_loan_remain_immovable_gage.xlsx', sheet_name='RawData')
...
# 2. 當 city_none = 1 時，將 immovable_worth 值帶入 city_none 欄位
loan_city_immovable_gage.loc[loan_city_immovable_gage['city_none'] == 1, 'city_none'] = loan_city_immovable_gage['immovable_worth'].astype(float)

# 3. 當 city_count = 1 時，將 loan_immovable_city 欄位中 = 1 的欄位名稱帶入 immovable_worth
mask_count1 = loan_city_immovable_gage['city_count'] == 1
for idx, row in loan_city_immovable_gage[mask_count1].iterrows():
    for city in loan_immovable_city:
        if row[city] == 1:
            loan_city_immovable_gage.at[idx, city] = row['immovable_worth']
        else:
            loan_city_immovable_gage.at[idx, city] = 0

# 4. 當 city_count > 1 時，將 immovable_worth/city_count 分配到所有 = 1 的城市欄位
mask_count_gt1 = loan_city_immovable_gage['city_count'] > 1
for idx, row in loan_city_immovable_gage[mask_count_gt1].iterrows():
    share = row['immovable_worth'] / row['city_count'] if row['city_count'] else 0
    for city in loan_immovable_city:
        if row[city] == 1:
            loan_city_immovable_gage.at[idx, city] = share
        else:
            loan_city_immovable_gage.at[idx, city] = 0


# === 建立以 loan_city_remain 分配的城市表 ===
...
# 2. 當 city_none = 1 時，將 loan_remain_capital 值帶入 city_none 欄位
loan_city_remain.loc[loan_city_remain['city_none'] == 1, 'city_none'] = loan_city_remain['loan_remain_capital']

# 3. 當 city_count = 1 時，將 loan_immovable_city 欄位中 = 1 的欄位名稱帶入 loan_remain_capital
mask_count1_remain = loan_city_remain['city_count'] == 1
for idx, row in loan_city_remain[mask_count1_remain].iterrows():
    for city in loan_immovable_city:
        if row[city] == 1:
            loan_city_remain.at[idx, city] = row['loan_remain_capital']
        else:
            loan_city_remain.at[idx, city] = 0

# 4. 當 city_count > 1 時，將 loan_remain_capital/city_count 分配到所有 =1 的城市欄位
mask_count_gt1_remain = loan_city_remain['city_count'] > 1
for idx, row in loan_city_remain[mask_count_gt1_remain].iterrows():
    share = row['loan_remain_capital'] / row['city_count'] if row['city_count'] else 0
    for city in loan_immovable_city:
        if row[city] == 1:
            loan_city_remain.at[idx, city] = share
        else:
            loan_city_remain.at[idx, city] = 0

loan_remain_merged = pd.merge(loan_remain_merged, loan_city_immovable_gage, on='loan_no', how='left', suffixes=('', '_worth'))
loan_remain_merged = pd.merge(loan_remain_merged, loan_city_remain, on='loan_no', how='left', suffixes=('', '_remain'))
...
table3 = loan_remain_merged.groupby(['pre_examine_no_return', 'loan_code_type'], as_index=False).agg({
    'loan_no': lambda x: x.nunique(),
    'loan_capital': 'sum', # 截至目前為止，尚存案件的原始的撥款金額
...
# 輸出四個表
with pd.ExcelWriter(r'./Result/Summary_截至20260430.xlsx') as writer:
