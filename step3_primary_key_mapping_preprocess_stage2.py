...
from data_function import prev_month_end

end_of_last_month = prev_month_end().strftime('%Y%m%d')

INPUT_STAGE1 = f"./Result/截至{end_of_last_month}/primary_key_mapping_stage1.xlsx"
INPUT_MANUAL = f"./Result/截至{end_of_last_month}/primary_key_mapping_manual.xlsx"
OUTPUT_FILE = f"./Result/截至{end_of_last_month}/primary_key_mapping.xlsx"
...
pre_manual_worth_col = pick_col(pre_manual, ["pre_immovable_worth_manual"])
pre_manual_sel = pre_manual[[pre_manual_col, pre_manual_worth_col]].rename(columns={pre_manual_col: "pre_examine_no", pre_manual_worth_col: "pre_immovable_worth_manual"})

# loan_manual 只取需要欄位
loan_manual_col = pick_col(loan_manual, ["loan_no", "loan_no_previous"])
loan_manual_worth_col = pick_col(loan_manual, ["immovable_worth_manual"])
...
pre_merged["pre_immovable_worth"] = np.where(
    pre_merged["pre_immovable_worth_manual"].notna() & (pre_merged["pre_immovable_worth_manual"] != ""),
    pre_merged["pre_immovable_worth_manual"],
